...
def query_gemini_text(prompt, model="models/gemini-1.5-flash-latest"):
    """Query Gemini for text completions."""
    genai.configure(api_key=os.getenv("GEMINI_API_KEY"))
...
def agentic_workflow(data_summary, feature_types):
    ...
    initial_insights = query_gemini_text(prompt)
    ...
    if "missing values" in initial_insights.lower():
        ...
        refinement = query_gemini_text(refinement_prompt)
...
def analyze_image_with_gemini(image_path, prompt="Analyze this image."):
